...
export function QuestionAnswerPage({
  ...
  const handleScreenshotTaken = async (imageSrc: string | null) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/storage/upload-webcam/${data.id ? "?testId=" + data.id : ""}`,
        {
          method: "POST",
          body: formData,
          headers: {
  ...
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(
    data.questions.findIndex((q) => q === data.currentQuestionId),
  )
  ...
  const renderQuestionContent = () => {
    ...
    const isDisabled =
      (currentQuestion.timeLimit === null && testTimeRemaining === 0) ||
        ...
        return (
          <RadioGroup
            ...
            onValueChange={(value) => {
              ...
              setSelectedAnswer(currentQuestion?.options[Number.parseInt(value)])
            }}
            disabled={isDisabled}
            className="space-y-2"
          >
            {currentQuestion.options?.map((option, index) => (
              ...
              >
                ...
                <Label htmlFor={`option-${index}`} className={`flex-grow ${cursorClass}`}>
                  {option}
                </Label>
              </div>
            ))}
        ...
        return (
          ...
          >
            ...
            >
              ...
              <Label htmlFor="true" className={`flex-grow ${cursorClass}`}>
                True
              </Label>
            </div>
            <div
            ...
            >
              ...
              <Label htmlFor="false" className={`flex-grow ${cursorClass}`}>
                False
              </Label>
            </div>
          </RadioGroup>
  ...
  const submitAnswer = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/tests/${data.id}/question/${data.questions[currentQuestionIndex]}/submit`,
        {
          method: "POST",
          headers: {
  ...
  if (isLoadingQuestion) {
    return (
      <div className="flex gap-2 h-screen items-center justify-center bg-white w-screen z-10">
        <Loader className="animate-spin" /> Loading next question...
      </div>
    )
  }
  if (isTestComplete) {
    return (
      <div className="flex items-center justify-center h-screen ">
        <Card className="lg:w-[25vw] border-green-200 bg-green-100 text-green-700 w-screen">
          ...
          <CardContent className="font-bold text-lg -mt-3">Submission successful</CardContent>
          <CardFooter className="text-sm -mt-3 flex flex-col w-full gap-2">
            {resultReady ? (
              <ResultsDialog accessToken={accessToken} testId={data.id} />
            ) : (
              <div className="flex items-start text-left w-full">You can close this page now.</div>
            )}
          </CardFooter>
        </Card>
      </div>
        ...
        <header className="bg-white shadow-md">
          <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
              {/* Title */}
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">{data.title}</h1>
                  ...
                  {currentQuestion?.media && (
                    <div className="space-y-2 flex items-center justify-center relative">
                      {currentQuestion.media.type === "image" ? (
                        <img
                          src={new URL(currentQuestion.media.url).toString() || "/placeholder.svg"}
                          width={400}
                          height={400}
                          alt="media"
                        />
